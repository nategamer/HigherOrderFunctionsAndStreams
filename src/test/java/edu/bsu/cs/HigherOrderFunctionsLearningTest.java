...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testMap() {
        ...
        List<Integer> actual = input.map(m -> m+1).collect(Collectors.toList());
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(m -> m.startsWith("b")).collect(Collectors.toList());
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(m -> m.replaceAll("[0-9]", "")).distinct().collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(m -> Integer.parseInt("" + m.charAt(m.length()-1)) % 2 == 0).reduce("",(partial, element) -> partial + element);
        System.out.println(actual);
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(r -> {
                System.out.println(r.user);
                return !r.user.matches("(?i)(.*)bot(.*)"); //returns opposite of match, to get the ones that dont have bot in name
        }).count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(r -> r.timestamp.atZone(ZoneId.systemDefault()).getMonth() == Month.FEBRUARY).count();
        int expected = 9;
        Assertions.assertEquals(expected, actual);
    }
    /**
